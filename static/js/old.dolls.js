...
const groupSelections = {
	...
	top: ["Default white button shirt", "Default white long sleeves", "Default white round collar"],
...
function loadDollAssets() {
    ...
    try {
        ...
        dollAssets = assetData
            .filter(asset => {
                const isValid = asset && typeof asset === 'object' && asset.group && asset.name && Array.isArray(asset.layers);
                if (!isValid) console.warn('Invalid asset skipped:', asset);
                return isValid;
            })
            .map(asset => {
...
function getLayers() {
    // Create a base order for groups to ensure consistent layering
    ...
    const layers = Object.entries(groupSelections)
        .flatMap(([group, itemNames]) => {
            // Get all selected items for this group
            const selectedAssets = dollAssets.filter(asset => {
                const isInGroup = asset.group === group || asset.baseGroup === group;
                const isSelected = itemNames.includes(asset.name);
            ...
            return selectedAssets.flatMap(asset => {
                ...
                // Convert asset to layers
                return asset.layers.map(layer => ({
                    layer: baseOrder + (layer.layer || 0) + (isBack ? -10000 : 0),
                    path: layer.path,
                    mask: !!color,
                    color
                }));
...
function getAvailableColorsForItem(itemName) {
    ...
    const colorVariants = dollAssets.filter(variant => {
        ...
        return assetMatch && 
               ...
               // For components, ensure we match the component type
               (!asset.isComponent || variant.componentType === asset.componentType);
    });
    
    colorVariants.forEach(asset => {
        const assetMatch = asset.name.match(colorRegex);
        if (assetMatch) {
            const colorName = assetMatch[2].toLowerCase();
...
function getAvailableColorsForCurrentGroup() {
    ...
    const groupAssets = dollAssets.filter(asset => 
...
function renderOptions() {
    ...
    componentTypes.forEach(type => {
        ...
        Object.values(groupedComponents).forEach(assetGroup => {
            ...
            // If this component is selected, show its color palette
                ...
                if (Object.keys(colors).length > 0) {
                    Object.entries(colors).forEach(([colorName, colorHex]) => {
                        ...
                        circle.addEventListener("click", () => {
                            ...
                            circle.classList.add("active");
                            setColorScheme(colorName, defaultAsset.name);
                        });
